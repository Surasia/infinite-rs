    ...
    Self: Seek,
{
    ...
    /// ```
    fn read_fixed_string(&mut self, length: usize) -> Result<String, Error> {
    ...
    /// ```
    fn read_enumerable<T: Default + Readable>(&mut self, count: u64) -> Result<Vec<T>, Error>
    where
        Self: Sized,
        Vec<T>: FromIterator<T>,
    {
        let enumerables = (0..count)
            .map(|_| -> Result<T, Error> {
